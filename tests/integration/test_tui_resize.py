    ...
    def test_60x20_semantic_reachability(self) -> None:
        async def scenario() -> None:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                # All three primary views remain mounted and primary controls
                # exist; no control was dropped by the narrow layout.
                for selector in (
                    "#session-note-input",
                    "#session-touched",
                    "#session-refresh",
    ...
    def test_narrow_stacks_session_action_row(self) -> None:
        async def scenario() -> None:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                await pilot.resize_terminal(60, 20)
                await _settle(pilot, lambda: app.screen.has_class("-w-narrow"))
                actions = app.query_one("#session-actions")
    ...
    def test_below_minimum_remains_scrollable(self) -> None:
        async def scenario() -> None:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                assert app.screen.styles.overflow_y == "auto"
                assert "vertical" in repr(app.query_one("#session-actions").styles.layout)
...
class TestResizeRoundTrip:
    ...
    def test_resize_with_expected_error_visible(self) -> None:
        async def scenario() -> None:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                await pilot.pause()
                app.query_one("#assistant-output", Static).update("Ошибка модели: сбой")
                await pilot.resize_terminal(60, 20)
                await pilot.pause()
                assert str(app.query_one("#assistant-output", Static).content) == (
                    "Ошибка модели: сбой"
                )
    ...
    def test_resize_with_campaign_state_visible(self) -> None:
        async def scenario() -> None:
            ...
            async with app.run_test(size=(100, 30)) as pilot:
                ...
                await pilot.pause()
                body_before = str(app.query_one("#campaign-state-body", Static).content)
                await pilot.resize_terminal(60, 20)
                await pilot.pause()
                assert str(app.query_one("#campaign-state-body", Static).content) == body_before
                assert app.query_one("#campaign-state-reload", Button).display
