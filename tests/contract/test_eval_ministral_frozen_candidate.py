"""Contract test: tracked S14-07-QUAL-02 frozen live Ollama candidate evidence.

The tracked JSON is the literal frozen output of the ONE measured live
product-v1 run for the distinct ``agent-ministral3-8b`` / ``ministral-3:8b``
candidate.  It is machine-consumed derived evidence, not campaign Source of
Truth, and must never be copied into the Vault.

The candidate was **not accepted** (4 runtime errors), so this contract binds a
frozen historical failure, not an accepted baseline.  Removing or replacing the
artifact without updating this contract must fail.
"""
...
ARTIFACT = (
    ...
    / "evidence"
    / "evals"
    / "s14-07-product-v1-ollama-ministral3-8b-candidate.json"
)

_EXPECTED_RUNTIME_ERRORS = 4
_EXPECTED_ERROR_SCENARIOS = frozenset({"EVAL-P1-002", "EVAL-P1-004", "EVAL-P1-006", "EVAL-P1-010"})
...
def test_runtime_identity_and_metadata_privacy() -> None:
    ...
    assert metadata["profile_name"] == "agent-ministral3-8b"
    assert metadata["model"] == "ministral-3:8b"
...
def test_failure_diagnostics_literal_candidate_outcome() -> None:
    report = _frozen_report()
    ...
    for observation in report.full_turn_observations:
        ...
        if observation.scenario_id in _EXPECTED_ERROR_SCENARIOS:
            ...
            assert diagnostic.cause_chain == ("ValidationError",)
        else:
