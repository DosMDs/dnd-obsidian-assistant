...
Orchestrate proportionally: implementation → architecture review → test/evidence review → corrections → final-diff review → finalization audit. Freeze edits while a reviewer inspects the frozen task snapshot; a correction invalidates affected review conclusions and requires a focused recheck. Each reviewer needs an independent context/subagent where available (never call a same-context pass independent). Give a reviewer only the minimal relevant inputs — base SHA, the exact diff including new files, scope/exclusions, acceptance map and command evidence — and never steer toward a preferred verdict. For trivial documentation changes one proportional combined pass suffices; report whether reviews were separate subagents or serial passes.
